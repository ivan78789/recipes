<header x-data="{ scrolled: false }" x-on:scroll.window="scrolled = (window.pageYOffset > 20)" :class="scrolled 
            ...
            : 'bg-gradient-to-r from-white/60 to-white/50 backdrop-blur-lg py-4 top-4'" class="fixed {{ (request()->is('profile*') || request()->is('dashboard')) && Auth::check() ? 'left-[296px]' : 'left-4' }} right-4 flex items-center justify-between px-8 
